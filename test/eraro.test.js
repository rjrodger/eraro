...
var x0 = new Error('x0')
var e5 = eraro(x0);
assert.equal("{ message: 'foo: x0', code: 'x0', package: 'foo', msg: 'foo: x0', details: { 'message$': 'x0' } }",
             descerr(e5))


var x1 = new Error('x1')
var e6 = eraro(x1,'c4');
assert.equal("{ message: 'foo: x1', code: 'c4', package: 'foo', msg: 'foo: x1', details: { 'message$': 'x1' } }",
             descerr(e6))
assert.equal(e6.orig,'Error: x1')

var x11 = new Error('x11')
var e61 = eraro(x11,'c401');
assert.equal("{ message: 'foo: C401 Message', code: 'c401', package: 'foo', msg: 'foo: C401 Message', details: { 'message$': 'x11' } }",
             descerr(e61))
...
assert.equal("{ message: 'foo: x2', code: 'c5', package: 'foo', msg: 'foo: x2', details: { a: 3, 'message$': 'x2' } }",
             descerr(e7))


var b0 = eraro('b0','<%=foo(1)%>')
assert.equal("foo: <%=foo(1)%> VALUES:{ code: \'b0\' } TEMPLATE ERROR: TypeError: foo is not a function",b0.message.replace(/string/,'foo'))
